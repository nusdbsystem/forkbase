...
TEST(LSTStore, Get) {
  ...
  for (int i = 0; i < NUMBER; ++i, (*val)++) {
    ustore::Chunk chunk(ustore::ChunkType::kBlob, sizeof(raw_data));
    std::copy(raw_data, raw_data + sizeof(raw_data), chunk.m_data());
    std::copy(chunk.hash().value(), chunk.hash().value() + ustore::Hash::kByteLength, hash[i]);
  }
  ...
  {
    // load from stroage
    const ustore::Chunk* c = cs->Get(ustore::Hash(reinterpret_cast<ustore::byte_t*>(hash[i])));
  ...
  LOG(INFO) << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - tp).count();
