...
void PerfmonDaemon::start(){
  ...
  while (!socket.svr_init(sock_port)){
    fprintf(stderr, "cannot create socket server at port %d\n, retry later...", sock_port);
    usleep(2000000);
  }
...
void PerfmonDaemon::processMessage(const char* hostname, int len){
  if (len % UNIT != 0){
    fprintf(stderr, "cannot process damaged message with length %d (unit length = %d)\n", len, UNIT);
    return;
  }
...
//  }
  ...
  PerfmonDaemon *daemon = new PerfmonDaemon(atoi(sock_port.c_str()), atoi(http_port.c_str()));
  daemon->start();
