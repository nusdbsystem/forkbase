...
bool SocketServer::svr_init(int port){
  ...
  if (setsockopt(socket_listen, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int))){
  ...
  //epoll control
  if (epoll_ctl(epollfd, EPOLL_CTL_ADD, socket_listen, &event) < 0){
...
void SocketServer::accept_conn(){
  ...
  char hostname[24];
  getnameinfo((struct sockaddr*)&sin, len, hostname, sizeof(hostname), NULL, 0, 0);
