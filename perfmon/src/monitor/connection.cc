 ...
 *
 *        Version:  1.0
 *        Created:  12/10/2014 12:05:09 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
void Connection::addPkt (Packet *pkt) {
    ...
    if (pkt->isOutgoing()) {
#ifdef PERF_DEBUG
        //std::cout << "capture an outgoing packet with length " << pkt->getLength() << "\n";
#endif
        lastOutgoingPkt_->addLength(pkt->getLength());
        totalPktSent_ += pkt->getLength();
    } else {
#ifdef PERF_DEBUG
        //std::cout << "capture an incoming packet with length " << pkt->getLength() << "\n";
#endif
...
void ConnectionManager::updateMap (char * buffer)
{
    ...
    else
    {
        ...
        for (std::vector<char *>::iterator it = ifaceName_.begin(); it != ifaceName_.end(); it++) {
            /* TODO maybe only add the ones with the same sa_family */
            snprintf(hashkey, HASHKEYSIZE * sizeof(char), "%s:%d-%s:%d", (*it), local_port, remote_string, rem_port);
 ...
 */
Connection *
ConnectionManager::findConnection (Packet * pkt)
{
 ...
 */
uint64_t
ConnectionManager::findInode (Connection * conn)
{
