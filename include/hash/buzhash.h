...
class BuzHash {
 public:
  explicit BuzHash(unsigned int n) {
    n_ = n;
    bshiftn_ = n % 32;
    bshiftm_ = 32 - bshiftn_;
    buf_ = new uint8_t[n];
    Reset();
  }
  ~BuzHash() { delete[] buf_; }
  ...
  uint32_t Sum32() { return state_; }

  uint32_t HashByte(uint8_t b) {
    if (bufpos_ == n_) {
      overflow_ = true;
      bufpos_ = 0;
    }
    ...
    if (overflow_) {
      uint32_t toshift = bytehash[buf_[bufpos_]];
      state ^= (toshift << bshiftn_) | (toshift >> bshiftm_);
    }
    buf_[bufpos_] = b;
    ++bufpos_;
    state ^= bytehash[b];
 ...
 private:
  uint32_t state_;
  unsigned int n_, bshiftn_, bshiftm_;
  size_t bufpos_;
  uint8_t* buf_;
  bool overflow_;
};
