...
//  public:
//   IndexComparator(const Hash& rhs,
//                   std::shared_ptr<ChunkLoader> loader) noexcept :
//       NodeComparator(rhs, loader) {}

//   std::vector<IndexRange> Intersect(const Hash& lhs) const override;

//   std::vector<IndexRange> Diff(const Hash& lhs) const override;

//  private:
//   // Precedure to scan element by element to perform diff or intersection
//   using IterateProcedure =
//       std::function<std::vector<IndexRange>(const Hash& lhs_root,
//                                             uint64_t lhs_start_idx,
//                                             const Hash& rhs_root,
...
//   // Precedure to perform when encounting the same hash
//   using IdenticalProcedure =
//       std::function<std::vector<IndexRange>(const SeqNode* lhs,
//                                             uint64_t lhs_start_idx)>;

//   // Return the hash of seq node in rhs, which is guanrateed to contain
//   // all the elements of index from lower to upper
//   // start_idx is a paramter to return the start_idx of this seqnode

// std::shared_ptr<const SeqNode> SmallestOverlap(uint64_t lower,
//                             uint64_t upper,
//                             std::shared_ptr<const SeqNode> rhs_seq_node,
//                             uint64_t rhs_start_idx,
//                             uint64_t* return_start_idx) const;

// std::vector<IndexRange> Compare(const SeqNode* lhs,
//                                 uint64_t lhs_start_idx,
//                                 std::shared_ptr<const SeqNode>
//                                     rhs_root_node,
//                                 uint64_t rhs_start_idx,
...
// class NodeKeyComparator : Noncopyable {
//  public:
//   // Use this constructor if compared by index
//   NodeKeyComparator(const Hash& rhs,
//                     std::shared_ptr<ChunkLoader> loader) noexcept;

//   std::vector<CursorRange> Intersect(const Hash& lhs) const;

//   std::vector<CursorRange> Diff(const Hash& lhs) const;

//  private:
//   using IterationProcedure =
//       std::function<std::vector<CursorRange>(const Hash&, const Hash&,
//                                              std::shared_ptr<ChunkLoader>)>;
...
//   static std::vector<CursorRange> IterateIntersect(const Hash& lhs,
//                                                    const Hash& rhs,
//                                                    std::shared_ptr<ChunkLoader>
//                                                        loader);

//   static std::vector<CursorRange> IterateDiff(const Hash& lhs,
//                                               const Hash& rhs,
//                                               std::shared_ptr<ChunkLoader>
//                                                   loader);
// };
