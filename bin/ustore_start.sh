...
for i in `cat $host_file` ; do
  ...
  if [ $host = localhost ] ; then
    echo Starting worker @ $host : $ustore_run --worker --node_id $i
    $ustore_run --worker --node_id $i > $USTORE_LOG/worker-$host-$port.log 2>&1 &
  else
    echo Starting worker @ $host : $ustore_sshrun --worker --node_id $i
    ssh $ssh_options $host $ustore_sshrun --worker --node_id $i >> $USTORE_LOG/worker-$host-$port.log 2>&1 &
  fi
done
