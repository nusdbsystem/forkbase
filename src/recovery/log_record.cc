...
char* LogRecord::ToString() {
		char ret[200];  // TODO: should be careful the out of bound error
		size_t pos = 0;
		...
		memcpy(ret+pos, &log_sequence_number_, sizeof(int64_t));
		pos = pos + sizeof(int64_t);
...
/*
	...
	* */
void LogRecord::ComputeChecksum() {
		checksum = version*12+logcmd*(log_sequence_number_%100)+key_length-value_length;
}
