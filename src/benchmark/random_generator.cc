...
namespace ustore {
  RandomGenerator::RandomGenerator() {
    ...
    alph_dist_ = std::uniform_int_distribution<>(0, sizeof(alphabet)/sizeof(*alphabet)-2);
  }
  ...
  void RandomGenerator::SequentialNumString(const int size, std::vector<std::string> *keys) {
    (*keys).reserve(size);
    int k=0;
