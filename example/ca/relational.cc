...
ErrorCode ColumnStore::CreateTable(const std::string& table_name) {
  ...
  // put new map
  VMeta put = odb_.Put(Slice(table_name), tmap,
...
ErrorCode ColumnStore::MergeTable(
  ...
  VMeta updated_table_meta = odb_.Merge(table_key, table_map,
                                        tgt_branch, ref_branch);
...
// Put the entire column as a Vlist
  ...
  // put new list
  VMeta col_meta = odb_.Put(Slice(table_col_key),
                            col_list, Slice(kDefaultBranch));
...
// Put the ucell hash of column list into tablemap
  ...
  table_map.Set(Slice(table_col_key), Slice(col_version_base32));
  VMeta updated_table_meta = odb_.Put(table_key,
                                      table_map,
                                      table_branch);
...
ErrorCode ColumnStore::DiffColumn(const std::string& lhs_table_name,
  ...
  USTORE_GUARD(GetColumn(lhs_table_name, lhs_branch_name,
                         lhs_col_name, &lhs_col));

  USTORE_GUARD(GetColumn(rhs_table_name, rhs_branch_name,
                         rhs_col_name, &rhs_col));
